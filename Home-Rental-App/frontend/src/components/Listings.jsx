...
const Listings = () => {
  ...
  // Use optional chaining to avoid issues if listings is null or undefined
  const listings = useSelector((state) => state?.listings?.listings) || []
  ...
  return (
    <>
      ...
      <div className="px-12 pb-32 lg:px-5 flex flex-wrap justify-center gap-5">
        {/* Check if listings is an array and has data */}
        {Array.isArray(listings) && listings.length > 0 ? (
          listings.map(
            ({
              ...
              <ListingCard
                key={_id} // Make sure to pass unique 'key' prop for each item
                listingId={_id}
                creator={creator}
                listingPhotoPaths={listingPhotoPaths}
                city={city}
